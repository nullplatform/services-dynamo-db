...
#
# dynamodb service worker image — the DynamoDB service built on the lean gRPC
# worker bridge. The bridge dials over gRPC and runs the bash entrypoint on
# each package-exec action; this image adds the cloud tooling the DynamoDB
# steps need and bakes the service in, so the channel needs no cmdline.
FROM public.ecr.aws/nullplatform/scopes/worker-bridge:1.0.0

# Tooling the DynamoDB workflows call (the bridge base stays minimal on
# purpose): aws + gomplate from apk. bash, jq, np, base64 and curl ship in the
# base.
