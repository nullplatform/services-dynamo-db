...
variable "role_name" {
  description = "Override for the DynamoDB permissions IAM role name. Defaults to nullplatform_{cluster_name}_dynamodb_role."
  type        = string
  default     = ""
}
...
variable "table_name_prefix" {
  description = "Prefix of the DynamoDB tables this service manages. The permissions are scoped to tables matching it, so the agent cannot reach tables created outside nullplatform."
  type        = string
  default     = "np-"
}
...
variable "state_bucket_name" {
  description = "Name of the existing S3 bucket holding the tofu state for every service instance, each under its own key prefix. Any bucket works — the module grants the role access to this one and assumes no naming convention. The agent receives the same name as DYNAMO_S3_STATE_BUCKET."
  type        = string
